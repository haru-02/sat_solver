...
let find_pure_symbol (cnf:Formula.cnf) : Formula.assignment option =
  let matches_assignment (lit:Formula.literal) (lit_lst:Formula.literal list) : bool option =
    List.fold_left (fun acc nxt -> match acc with
  ...
  let pure_lits = List.fold_left (fun pure_lits nxt_clause ->
      (List.fold_left (fun lits nxt_lit ->
...
let find_unit_clause (cnf:Formula.cnf) : Formula.assignment option =
...
let cleanup (cnf:Formula.cnf) (sym:Formula.variable) : Formula.cnf =
  List.fold_left (fun new_cnf clause ->
    let new_clause = (List.fold_left (fun new_clause lit ->
      ...
      match new_clause with 
        [] -> new_cnf 
        | _ -> new_clause::new_cnf) [] cnf

(* main method to run dpll algorithm: returns (bool, model) where bool
 * = 'is cnf satisfied by model' *)
let dpll (cnf:Formula.cnf) : bool*Formula.model =
  let rec dpll_sat (cnf:Formula.cnf) (syms:Formula.variable list) (ml:Formula.model) : bool*Formula.model =
    if (Formula.is_cnf_satisfied cnf ml) then (true, ml)
    else
  ...
  dpll_sat cnf (Formula.variables_in_cnf cnf) []
